...
def verify_webhook_signature(raw_body: bytes, signature_header: Optional[str]) -> bool:
    """
    Korapay signs webhooks with HMAC-SHA256 of ONLY the `data` object inside the
    payload (not the whole raw body — that's the bug this replaced: hashing the
    full body with the "event" wrapper included never matches Korapay's
    signature, so every real webhook was silently rejected with 401 and the
    wallet was never credited). See https://developers.korapay.com/docs/webhooks.
    """
    ...
    # keys in original order. json.dumps with separators=(",", ":") matches
    # that exactly as long as we don't re-sort the keys.
    serialized = json.dumps(data_obj, separators=(",", ":"))
    expected = hmac.new(
        settings.korapay_secret_key.encode(), serialized.encode(), hashlib.sha256
    ).hexdigest()
