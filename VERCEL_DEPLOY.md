...
├── ryzel-site/            <- your existing Next.js project, unchanged location
│   └── ...
└── ryzel-api/            <- your existing FastAPI project, unchanged location
    └── ...
```
...
- `entrypoint: "app.main:app"` points at the `app` FastAPI instance in
  `ryzel-api/app/main.py` — already correct, no backend code changes needed.
...
- `DATABASE_URL` — Supabase's **pooled** connection string (port 6543,
  ...
  `ryzel-api/.env.example` for why this matters specifically for serverless.
- `SUPABASE_JWT_SECRET`
