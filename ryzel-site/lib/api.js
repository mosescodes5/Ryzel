...
export const listInvoices = () => api("/invoices");

export const getInvoice = (id) => api(`/invoices/${id}`);

export const createInvoice = (payload) =>
  apiWithBody("/invoices", { body: payload });

export const updateInvoiceStatus = (id, status) =>
  apiWithBody(`/invoices/${id}/status`, {
    method: "PATCH",
    body: { status },
  });
...
export const listTrackers = () => api("/trackers");

export const getTracker = (id) => api(`/trackers/${id}`);

export const createTracker = (payload) =>
  apiWithBody("/trackers", { body: payload });

export const addTrackerEvent = (id, payload) =>
  apiWithBody(`/trackers/${id}/events`, { body: payload });

export const deleteTracker = (id) =>
  api(`/trackers/${id}`, { method: "DELETE" });

/**
 * Public tracking lookup — no auth, callable from the unauthenticated
 * /track/[code] page.
 */
export const getPublicTracker = (code) =>
  api(`/track/${code}`, { auth: false });
...
/**
 * Poll this after Korapay redirects back — the wallet is credited
 * server-side by the webhook, not by the browser landing here, so this
 * just checks whether that's finished yet.
 */
export const getPaymentStatus = (reference) =>
  api(`/payments/korapay/status/${encodeURIComponent(reference)}`);